...
def create(model_name: str, load_pretrain: bool, num_classes: int) -> nn.Module:
    ...
    elif model_name == 'x3d_m_dropout':
        return create_x3d_m_dropout(load_pretrain, num_classes)
    ...
    elif model_name == 'sagcn':
        return create_sagcn(load_pretrain, num_classes)
    else:
...
def create_sagcn(load_pretrain, num_classes):
    model_path = '/data/models/2sagcn_80e_ntu60_xsub_keypoint_3d-3bed61ba.pth'
...
def create_x3d_m_dropout(load_pretrain, num_classes):
    model = torch.hub.load('facebookresearch/pytorchvideo', 'x3d_m', pretrained=load_pretrain)
...
def load_video_model(model_name: str, load_pretrain: bool, num_classes: int, embedding_size: int, load_path: str) -> nn.Module:
    model = create(model_name, load_pretrain, num_classes)
    model.load_state_dict(torch.load(load_path))
    model[-1] = nn.Linear(2048, embedding_size, bias=True)
    return model