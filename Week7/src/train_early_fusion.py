...
TRAIN_EMBEDDINGS_PATH = "/ghome/group01/mcv-c6-2024-team1/Week7/results/embeddings_posec3d_train.pkl"
TEST_EMBEDDINGS_PATH  = "/ghome/group01/mcv-c6-2024-team1/Week7/results/embeddings_posec3d_test.pkl"


def load_embeddings(train_path: str, test_path: str) -> Tuple[Dict, Dict]:
    with open(train_path, "rb") as f:
        train_embeddings_keypoints = pickle.load(f)

    with open(test_path, "rb") as f:
        test_embeddings_keypoints = pickle.load(f)

    return train_embeddings_keypoints, test_embeddings_keypoints
...
def train(
    ...
    device: str,
    description: str = "",
    train_embeddings_keypoints: Dict = None,
) -> None:
    """
    ...
    Args:
        ...
        train_embeddings_keypoints (Dict): Dictionary containing train embeddings from keypoints.
    Returns:
        None
    """
    model.train()
    ...
    for batch in pbar:
        ...
        # embeddings = [train_embeddings_keypoints[path] for path in paths]  # TODO: PASARLO A GPU
        embeddings = torch.stack(
            [torch.FloatTensor(train_embeddings_keypoints[path]).to(device) for path in paths]
        ).to(device)
...
if __name__ == "__main__":
    ...
    wandb.init(project="C6_w7_multimodal", config=args)

    train_embeddings_keypoints, test_embeddings_keypoints = load_embeddings(TRAIN_EMBEDDINGS_PATH, TEST_EMBEDDINGS_PATH)
    ...
    for epoch in range(args.epochs):
        ...
        train_loss, train_acc = train(
            ...
            args.device,
            description=description,
            train_embeddings_keypoints=train_embeddings_keypoints
        )
