...
class HMDB51Dataset_RGB_difference(Dataset):
    """
    Dataset class for HMDB51 dataset.
    """
    ...
    def _read_annotation(self) -> pd.DataFrame:
        ...
        annotation = []
        for class_name in HMDB51Dataset_RGB_difference.CLASS_NAMES:
            ...
            df['class_id'] = HMDB51Dataset_RGB_difference.CLASS_NAMES.index(class_name)
            annotation += [df]
    ...
    def _create_transform(self) -> v2.Compose:
        ...
        """
        if self.regime == HMDB51Dataset_RGB_difference.Regime.TRAINING:
            return v2.Compose([
    ...
    def get_num_classes(self) -> int:
        ...
        """
        return len(HMDB51Dataset_RGB_difference.CLASS_NAMES)
    ...
    def __getitem__(self, idx: int) -> tuple:
        ...
        video_diff = torch.zeros_like(video)
        for i in range(1, len(video)):
            video_diff[i] = video[i] - video[i - 1]

        # Special case: first frame, set difference to difference from the previous frame outside the clip
        if clip_begin > 0:
            frame_prev = read_image(frame_paths[clip_begin - 1])
            video_diff[0] = frame.clone().detach() - frame_prev.clone().detach()

        # Special case: last frame, set difference to difference from the following frame outside the clip
        if clip_end < video_len:
            frame_next = read_image(frame_paths[clip_end])
            video_diff[-1] = frame_next.clone().detach() - frame.clone().detach()


        label = df_idx['class_id']
        assert video_diff is not None

        if self.shuffled:
            video_diff = video_diff[torch.randperm(video_diff.shape[0])]

        return video_diff, label, video_path
    ...
    def collate_fn(self, batch: list) -> dict:
        ...
        if self.permute:
            ...
            transformed_clips = [self.transform(clip) for clip in unbatched_clips]
