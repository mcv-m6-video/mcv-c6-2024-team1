...
trackings_path = "./Week3/trackings"
tracker_folder = "./TrackEval/data/trackers/mot_challenge/aicity19-train"

for file in Path(trackings_path).glob("*.json"):
    trackername = "".join(file.stem.split("_")[:2])
    seq_name = "".join(file.stem.split("_")[-3:-1])
    seq_file = Path(tracker_folder) / trackername / "data" / f"{seq_name}.txt"
    seq_file.parent.mkdir(parents=True, exist_ok=True)

    with open(file) as f:
        bbxs = json.load(f)
    write_bbxs_to_csv(bbxs, seq_file)