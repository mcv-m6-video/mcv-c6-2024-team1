...
class AdaptativeGaussianModel:
    ...
    def updateBackground(self, foreground:np.ndarray,frame: np.ndarray , rho: float = 0.1):
        updated_mean = rho * frame + (1 - rho) * self.background_mean
        updated_var = rho * (frame - self.background_mean) ** 2 + (1 - rho) * self.background_std**2
    ...
    def segment(self, alpha: float):
        ...
        for _ in tqdm(range(test_frames), desc="Predicting test frames"):
            ...
            #cv2.imshow('img', (self.background_mean).astype(np.uint8))
            #cv2.waitKey(0)
            frame_id = str(int(self.video.get(cv2.CAP_PROP_POS_FRAMES)) - 1)
            gray_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
            foreground = np.abs(gray_frame - self.background_mean) >= alpha * (self.background_std + 2)
            ...
            #cv2.imshow('img', postprocessed_foreground)
            #cv2.waitKey(0)
            prediction,binary_colored,mask = self.detect_object(postprocessed_foreground)
            background_img = postprocessed_foreground

            if self.use_mask: #only take into account as foreground what has been detected as object
                background_img= mask 
            ...
            predictions.update({frame_id: prediction})
            frames.append(cv2.cvtColor(postprocessed_foreground, cv2.COLOR_GRAY2RGB))
            background.append(cv2.cvtColor(self.background_mean.astype(np.uint8), cv2.COLOR_GRAY2RGB))
